...
static void reassociate_mul_minus_one(
	...
	if (def_opline->opcode == ZEND_MUL || def_opline->opcode == ZEND_DIV) {
		...
		/* Remove def of MUL/DIV result from SSA */
		remove_uses_of_var(ssa, var_num);
		remove_result_def(ssa, def_op);

		/* Move def of the MUL -1 to the MUL/DIV op */
		COPY_NODE(def_opline->result, opline->result);
		def_op->result_def = var_num;
		ssa_op->result_def = -1;
		ssa->vars[def_op->result_def].definition = def_op - ssa->ops;

		/* Finally, remove the MUL -1 instruction */
