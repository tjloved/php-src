...
static void handle_assign(
		zend_op_array *op_array, zend_ssa *ssa, zend_ssa_op *ssa_op, zend_op *opline, int i) {
	if (opline->op1_type != IS_CV || RETURN_VALUE_USED(opline)) {
		return;
	}
...
void ssa_optimize_assign(ssa_opt_ctx *ctx) {
	...
	FOREACH_INSTR_NUM(i) {
		...
		if (opline->opcode == ZEND_ASSIGN) {
			handle_assign(op_array, ssa, ssa_op, opline, i);
		}

		/* Optimize ASSIGN_ADD $x, OP into $x = ADD $x, OP if $x is not refcounted */
		if (is_compound_assign(opline)) {
			if (opline->op1_type != IS_CV
					|| RETURN_VALUE_USED(opline)
					|| opline->extended_value
					|| (ssa->var_info[ssa_op->op1_use].type & (MAY_BE_REF|MAY_BE_REFCOUNTED))) {
				continue;
			}
