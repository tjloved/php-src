...
static void handle_type_narrowing(const zend_op_array *op_array, zend_ssa *ssa, int var, uint32_t old_type, uint32_t new_type)
{
	if (1) {
...
static void zend_update_type_info(
	...
	if (1 && ctx->combine_scp) {
		...
		} else if (ssa_ops[i].op1_use >= 0
				&& scp_value_known(&ctx->scp.values[ssa_ops[i].op1_use])) {
		...
		} else if (ssa_ops[i].op2_use >= 0
				&& scp_value_known(&ctx->scp.values[ssa_ops[i].op2_use])) {
...
static void zend_update_phi_type_info(scdf_ctx *scdf, void *void_ctx, zend_ssa_phi *p) {
	zend_ssa *ssa = scdf->ssa;
	const zend_op_array *op_array = scdf->op_array;
	zend_ssa_var *ssa_vars = ssa->vars;
	zend_ssa_var_info *ssa_var_info = ssa->var_info;
	...
	if (p->pi >= 0) {
		...
		for (i = 0; i < block->predecessors_count; i++) {
			if (p->sources[i] >= 0
					&& scdf_is_edge_feasible(scdf, predecessors[i], p->block)) {
...
static int zend_type_narrowing(const zend_op_array *op_array, const zend_script *script, zend_ssa *ssa, scdf_ctx *scdf)
{
	...
	scdf_solve(scdf, "Type inference (after narrowing)");
...
static int zend_infer_types(
		const zend_op_array *op_array, const zend_script *script, zend_ssa *ssa)
{
	zend_ssa_var_info *ssa_var_info = ssa->var_info;
	int ssa_vars_count = ssa->vars_count;
	int j;
	...
	if (ctx.combine_scp) {
		zend_call_info **call_map = ZEND_FUNC_INFO(op_array)->call_map;
