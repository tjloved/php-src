...
static int php_array_key_compare(const void *a, const void *b) /* {{{ */
{
	...
	if (!zend_bucket_has_str_key(f)) {
		ZVAL_LONG(&first, f->key.num);
	} else {
		ZVAL_STR(&first, f->key.str);
	}

	if (!zend_bucket_has_str_key(s)) {
		ZVAL_LONG(&second, s->key.num);
	} else {
		ZVAL_STR(&second, s->key.str);
	}
...
static void php_array_intersect_key(INTERNAL_FUNCTION_PARAMETERS, int data_compare_type) /* {{{ */
{
	...
	for (idx = 0; idx < Z_ARRVAL(args[0])->nNumUsed; idx++) {
		...
		if (!zend_bucket_has_str_key(p)) {
			...
			ok = 1;
			for (i = 1; i < argc; i++) {
				if ((data = zend_hash_find(Z_ARRVAL(args[i]), p->key.str)) == NULL ||
					(intersect_data_compare_func &&
					intersect_data_compare_func(val, data) != 0)
				) {
					ok = 0;
	...
	for (i = 0; i < num_arrays; i++) {
		...
		for (n = 0, k = 0; k < array_size; k++) {
			...
			if (!zend_bucket_has_str_key(&hash->arData[k])) {
				zend_bucket_set_int_key(&hash->arData[k], n++);
			} else {
				repack = 0;
			}
		}
