	...
	if (OP2_TYPE != IS_CONST ||
		...
		if (UNEXPECTED(fbc == NULL)) {
			zend_throw_engine_exception_ex("Call to undefined method %s::%s()" TSRMLS_CC,
				obj->ce->name->val, Z_STRVAL_P(function_name));
			FREE_OP2();
			FREE_OP1();
			HANDLE_EXCEPTION();
		}
	...
	if (!(fbc->common.fn_flags & ZEND_ACC_STATIC)) {
		...
		if (!object ||
			...
			if (fbc->common.fn_flags & ZEND_ACC_ALLOW_STATIC) {
				...
				zend_throw_engine_exception_ex(
					"Non-static method %s::%s() cannot be called statically%s" TSRMLS_CC,
					fbc->common.scope->name->val, fbc->common.function_name->val,
					object ? ", assuming $this from incompatible context" : "");
				HANDLE_EXCEPTION();
			}
	...
	if (OP2_TYPE == IS_CONST && Z_TYPE_P(opline->op2.zv) == IS_STRING) {
		...
		if (EXPECTED(Z_TYPE_P(function_name) == IS_STRING)) {
			...
			if (Z_TYPE_P(obj) == IS_STRING) {
				...
				if (UNEXPECTED(fbc == NULL)) {
					zend_throw_engine_exception_ex("Call to undefined method %s::%s()" TSRMLS_CC,
						object->ce->name->val, Z_STRVAL_P(method));
					FREE_OP2();
					HANDLE_EXCEPTION();
				}
...
ZEND_VM_HANDLER(68, ZEND_NEW, CONST|VAR, ANY)
{
	...
	if (UNEXPECTED((ce->ce_flags & (ZEND_ACC_INTERFACE|ZEND_ACC_IMPLICIT_ABSTRACT_CLASS|ZEND_ACC_EXPLICIT_ABSTRACT_CLASS)) != 0)) {
		if (ce->ce_flags & ZEND_ACC_INTERFACE) {
			zend_throw_engine_exception_ex(
				"Cannot instantiate interface %s" TSRMLS_CC, ce->name->val);
		} else if ((ce->ce_flags & ZEND_ACC_TRAIT) == ZEND_ACC_TRAIT) {
			zend_throw_engine_exception_ex(
				"Cannot instantiate trait %s" TSRMLS_CC, ce->name->val);
		} else {
			zend_throw_engine_exception_ex(
				"Cannot instantiate abstract class %s" TSRMLS_CC, ce->name->val);
		}
		HANDLE_EXCEPTION();
	}
