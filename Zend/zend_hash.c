...
#define HT_TYPE_FIND   0
#define HT_TYPE_STORE  1
#define HT_TYPE_DELETE 2

#define HT_VALID_IDX(idx) (idx < DELETED_IDX)

/* Top bit of hash is key type */
#define HT_HAS_STR_KEY(p) ((Z_HASH((p)->val) & (1 << 31)) != 0)
#define HT_INT_HASH(num) (num & ~(1 << 31))

#define HT_KEY_MATCHES(p, _key, _h) ((p)->key.str == (_key) || \
	(Z_HASH((p)->val) == (_h) && (p)->key.str->len == (_key)->len \
	 && memcmp((p)->key.str->val, (_key)->val, (_key)->len) == 0))
#define HT_STR_KEY_MATCHES(p, _str, _len, _h) \
	(Z_HASH((p)->val) == (_h) && (p)->key.str->len == (_len) \
	 && memcmp((p)->key.str->val, (_str), (_len)) == 0)
...
#define CHECK_INIT(ht, packed) do { \
	if (UNEXPECTED(!((ht)->u.flags & HASH_FLAG_INITIALIZED))) { \
		if (packed) { \
			...
			(ht)->arData = (Bucket *) safe_pemalloc((ht)->nTableSize, sizeof(Bucket), 0, (ht)->u.flags & HASH_FLAG_PERSISTENT); \
			(ht)->arHash = safe_pemalloc((ht)->nTableSize, 2 * sizeof(uint32_t), 0, (ht)->u.flags & HASH_FLAG_PERSISTENT); \
			(ht)->nResizeCounter = 2 * 2 * (ht)->nTableSize; \
...
ZEND_API void zend_hash_packed_to_hash(HashTable *ht)
{
	...
	ht->nTableMask = 2 * ht->nTableSize - 1;
	ht->nResizeCounter = 2 * 2 * ht->nTableSize - 3 * ht->nNumOfElements;
	ht->arHash = (uint32_t *) safe_pemalloc(ht->nTableSize, 2 * sizeof(uint32_t), 0, ht->u.flags & HASH_FLAG_PERSISTENT);
	zend_hash_rehash(ht);
	HANDLE_UNBLOCK_INTERRUPTIONS();
}
...
ZEND_API void zend_hash_to_packed(HashTable *ht)
{
	...
	pefree(ht->arHash, ht->u.flags & HASH_FLAG_PERSISTENT);
	HANDLE_UNBLOCK_INTERRUPTIONS();
}
...
static zend_always_inline void _zend_hash_insert_size_check(HashTable *ht, uint32_t h) {
	...
	ht->nResizeCounter -= 3;
	if (rehashed) {
		_zend_hash_fast_insert(ht, h, ht->nNumUsed);
	}
}
...
static zend_always_inline zval *_zend_hash_add_or_update_i(HashTable *ht, zend_string *key, zval *pData, uint32_t flag ZEND_FILE_LINE_DC)
{
	...
	_zend_hash_insert_size_check(ht, h);
...
add_to_hash:
	...
	_zend_hash_insert_size_check(ht, HT_INT_HASH(h));
...
static void zend_hash_grow_hash(HashTable *ht) {
	uint32_t size = ht->nTableMask + 1;
	if (2 * ht->nNumOfElements >= size) { // TODO.OA
		ht->arHash = safe_perealloc(ht->arHash, size * 2, sizeof(uint32_t), 0, ht->u.flags & HASH_FLAG_PERSISTENT);
		ht->nTableMask = 2 * size - 1;
		ht->nResizeCounter = 2 * 2 * size - 3 * ht->nNumOfElements;
	} else {
		ht->nResizeCounter = 2 * size - 3 * ht->nNumOfElements;
	}
	zend_hash_rehash(ht);
}
...
ZEND_API int zend_hash_rehash(HashTable *ht)
{
	...
	if (UNEXPECTED(ht->nNumOfElements == 0)) {
		if (ht->u.flags & HASH_FLAG_INITIALIZED) {
			memset(ht->arHash, INVALID_IDX, (ht->nTableMask + 1) * sizeof(uint32_t));
		}
		return SUCCESS;
	}
	...
	for (i = 0, j = 0; i < ht->nNumUsed; i++) {
		...
		_zend_hash_fast_insert(ht, Z_HASH(p->val), j++);
	}
	ht->nNumUsed = j;
	return SUCCESS;
}

static zend_always_inline void _zend_hash_del_el_ex(HashTable *ht, uint32_t idx, Bucket *p)
{
	HANDLE_BLOCK_INTERRUPTIONS();
...
ZEND_API int zend_hash_del(HashTable *ht, zend_string *key)
{
	...
	_zend_hash_del_el_ex(ht, p - ht->arData, p);
	return SUCCESS;
}
...
ZEND_API void zend_hash_clean(HashTable *ht)
{
	...
	ht->nNumOfElements = 0;
	ht->nNextFreeElement = 0;
	ht->nResizeCounter = 2 * (ht->nTableMask + 1);
	ht->nInternalPointer = INVALID_IDX;
}
...
ZEND_API void zend_hash_apply_with_arguments(HashTable *ht, apply_func_args_t apply_func, int num_args, ...)
{
	...
	for (idx = 0; idx < ht->nNumUsed; idx++) {
		...
		if (HT_HAS_STR_KEY(p)) {
			hash_key.key = p->key.str;
			hash_key.h = 0;
		} else {
			hash_key.key = NULL;
			hash_key.h = p->key.num;
		}
...
ZEND_API void zend_array_dup(HashTable *target, HashTable *source)
{
	...
	if (target->u.flags & HASH_FLAG_INITIALIZED) {
		if (target->u.flags & HASH_FLAG_PACKED) {
			...
			target->nNextFreeElement = source->nNextFreeElement;
			target->nResizeCounter = 2 * (target->nTableMask + 1);
			target->arData = (Bucket *) safe_pemalloc(target->nTableSize, sizeof(Bucket), 0, 0);
			target->arHash = (uint32_t *) safe_pemalloc(target->nTableMask + 1, sizeof(uint32_t), 0, 0);
			memset(target->arHash, INVALID_IDX, (target->nTableMask + 1) * sizeof(uint32_t));
			...
			for (idx = 0; idx < source->nNumUsed; idx++) {
				...
				_zend_hash_fast_insert(target, Z_HASH(p->val), target_idx++);
				if (Z_OPT_REFCOUNTED_P(data)) {
					if (Z_ISREF_P(data) && Z_REFCOUNT_P(data) == 1) {
						ZVAL_COPY(&q->val, Z_REFVAL_P(data));
					} else {
...
ZEND_API void _zend_hash_merge(HashTable *target, HashTable *source, copy_ctor_func_t pCopyConstructor, zend_bool overwrite ZEND_FILE_LINE_DC)
{
	...
	for (idx = 0; idx < source->nNumUsed; idx++) {
		...
		if (Z_TYPE(p->val) == IS_UNDEF) continue;
		if (HT_HAS_STR_KEY(p)) {
			t = _zend_hash_add_or_update(target, p->key.str, &p->val, mode ZEND_FILE_LINE_RELAY_CC);
			if (t && pCopyConstructor) {
				pCopyConstructor(t);
			}
		} else {
			if ((mode==HASH_UPDATE || !zend_hash_index_exists(target, p->key.num))) {
...
// TODO.OA Same as non-packed now
...
ZEND_API int zend_hash_compare(HashTable *ht1, HashTable *ht2, compare_func_t compar, zend_bool ordered)
{
	...
	for (idx1 = 0, idx2 = 0; idx1 < ht1->nNumUsed; idx1++) {
		...
		if (ordered) {
			...
			if (!HT_HAS_STR_KEY(p1) && !HT_HAS_STR_KEY(p1)) { /* numeric indices */
				result = p1->key.num - p2->key.num;
				if (result != 0) {
					HASH_UNPROTECT_RECURSION(ht1);
					HASH_UNPROTECT_RECURSION(ht2);
					return result;
				...
				size_t len0 = (p1->key.str ? p1->key.str->len : 0);
				size_t len1 = (p2->key.str ? p2->key.str->len : 0);
				if (len0 != len1) {
					HASH_UNPROTECT_RECURSION(ht1);
					HASH_UNPROTECT_RECURSION(ht2);
					return len0 > len1 ? 1 : -1;
				}
				result = memcmp(p1->key.str->val, p2->key.str->val, p1->key.str->len);
				if (result != 0) {
					HASH_UNPROTECT_RECURSION(ht1);
					HASH_UNPROTECT_RECURSION(ht2);
					return result;
